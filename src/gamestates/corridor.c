...
void Gamestate_Logic(struct Game *game, struct GamestateResources* data) {
	...
	if (data->movedown) {
		...
		if (GetCharacterX(game, data->ego) > 700) {
	...
	if (GetCharacterY(game, data->ego) < 500) {
		//SetCharacterPosition(game, data->ego, GetCharacterX(game, data->ego), 500, 0);
	...
	if (GetCharacterX(game, data->ego) < 400) {
		SetCharacterPosition(game, data->ego, 400, GetCharacterY(game, data->ego), 0);
	}
...
void* Gamestate_Load(struct Game *game, void (*progress)(struct Game*)) {
	...
	data->bg = al_load_bitmap(GetDataFilePath(game, "kory.png"));
...
void Gamestate_Start(struct Game *game, struct GamestateResources* data) {
	...
	// playing music etc.
	SayDialog(game, game->data->faceg, "This new von Wissenschäftler's weapon looks even more impressive than the last 42 of them!", "new");
